(()=>{var rt=Object.defineProperty;var o=(e,t)=>()=>(e&&(t=e(e=0)),t);var a=(e,t)=>{for(var i in t)rt(e,i,{get:t[i],enumerable:!0})};var A=o(()=>{});var k,y=o(()=>{k=`{% block heptacom_admin_open_auth_scope_field %}
...
{% endblock %}
`});var j={};a(j,{default:()=>vt});var vt,z=o(()=>{H();E();vt={template:N,inject:["repositoryFactory","heptacomOauthRuleDataProvider"],props:{isLoading:{required:!1,type:Boolean,default:!1},client:{required:!0,type:Object},rule:{required:!0,type:Object}},computed:{conditionRepository(){return this.repositoryFactory.create("heptacom_admin_open_auth_client_rule_condition")},scopes(){return["global",this.client.provider]}},methods:{onConditionsChanged({deletedIds:e}){e.length>0&&this.$emit("conditions-deleted",e)},onMoveUp(){this.$emit("move-up",{})},onMoveDown(){this.$emit("move-down",{})},onDelete(){this.$emit("delete",{})}}}});var B=o(()=>{});var X,K=o(()=>{X=`{% block heptacom_admin_open_auth_client_create_page %}
    <sw-page
        :showSmartBar="false"
        :showSearchBar="false"
...
{% endblock %}
`});var ee={};a(ee,{default:()=>kt});var c,Q,yt,Y,kt,te=o(()=>{V();W();({Context:c,Mixin:Q,Data:yt}=Shopware),{Criteria:Y}=yt,kt={template:Z,inject:["acl","repositoryFactory","HeptacomAdminOpenAuthProviderApiService"],mixins:[Q.getByName("placeholder"),Q.getByName("notification")],props:{clientId:{required:!0,type:String}},data(){return{isLoading:!0,isSaveSuccessful:!1,item:null,showDeleteModal:!1,redirectUri:null,metadataUri:null,deletedRuleIds:[],deletedConditionIds:[]}},created(){this.loadData()},computed:{dynamicName(){return this.placeholder(this.item,"name",this.$t("heptacom-admin-open-auth-client.pages.edit.title"))},clientRepository(){return this.repositoryFactory.create("heptacom_admin_open_auth_client")},ruleRepository(){return this.repositoryFactory.create("heptacom_admin_open_auth_client_rule")},ruleConditionRepository(){return this.repositoryFactory.create("heptacom_admin_open_auth_client_rule_condition")},clientCriteria(){let e=new Y;return e.getAssociation("rules").addSorting(Y.sort("position","ASC")),e.addAssociation("rules.aclRoles"),e.addAssociation("rules.conditions"),e.addAssociation("defaultAclRoles"),e},providerSlug(){return(this.item&&this.item.provider?this.item.provider:"").replace(/_/g,"-")},providerSettingsComponent(){return`heptacom-admin-open-auth-provider-${this.providerSlug}-settings`},providerSettingsProps(){return{item:this.item}}},methods:{loadData(){this.isLoading=!0,this.loadClient().finally(()=>{this.isLoading=!1})},async loadClient(){this.item=null,this.item=await this.clientRepository.get(this.clientId,c.api,this.clientCriteria),this.redirectUri=(await this.HeptacomAdminOpenAuthProviderApiService.getRedirectUri(this.item.id)).target,this.metadataUri=(await this.HeptacomAdminOpenAuthProviderApiService.getMetadataUri(this.item.id)).target},cancelEdit(){this.$router.push({name:this.$route.meta.parentPath})},saveItem(){this.isLoading=!0,this.saveClient().then(this.syncDeletedConditions).then(this.syncDeletedRules).then(()=>(this.isSaveSuccessful=!0,this.loadData())).catch(e=>{let t=this.client.name;throw this.createNotificationError({title:this.$tc("global.notification.notificationSaveErrorTitle"),message:this.$tc("global.notification.notificationSaveErrorMessage",0,{entityName:t})}),e}).finally(()=>{this.isLoading=!1})},saveClient(){return this.clientRepository.save(this.item,c.api)},syncDeletedRules(){if(this.deletedRuleIds.length>0)return this.ruleRepository.syncDeleted(this.deletedRuleIds,c.api).then(()=>{this.deletedRuleIds=[]})},syncDeletedConditions(){if(this.deletedConditionIds.length>0)return this.ruleConditionRepository.syncDeleted(this.deletedConditionIds,c.api).then(()=>{this.deletedConditionIds=[]})},onRuleDeleted(e){this.deletedRuleIds.push(e)},onConditionsDeleted(e){this.deletedConditionIds=[...this.deletedConditionIds,...e]},onConfirmDelete(){return this.showDeleteModal=!1,this.isLoading=!0,this.clientRepository.delete(this.item.id,c.api).then(()=>{this.$router.push({name:"heptacom.admin.open.auth.client.settings"})}).catch(e=>{let t=this.client.name;throw this.createNotificationError({title:this.$tc("global.notification.notificationSaveErrorTitle"),message:this.$tc("global.notification.notificationSaveErrorMessage",0,{entityName:t})}),e}).finally(()=>{this.isLoading=!1})}}}});var ie=o(()=>{});var ne,oe=o(()=>{ne=`{% block heptacom_admin_open_auth_client_listing_page %}
...
{% endblock %}
`});var ae={};a(ae,{default:()=>$t});var xt,Ot,St,v,$t,re=o(()=>{ie();oe();({Context:xt,Data:Ot,Mixin:St}=Shopware),{Criteria:v}=Ot,$t={template:ne,inject:["acl","repositoryFactory"],mixins:[St.getByName("listing")],data(){return{isLoading:!0,items:null,columns:[{property:"active",label:this.$t("heptacom-admin-open-auth-client.pages.listing.columns.active"),allowResize:!1,width:"50px"},{property:"name",label:this.$t("heptacom-admin-open-auth-client.pages.listing.columns.name"),routerLink:"heptacom.admin.open.auth.client.edit"},{property:"provider",label:this.$t("heptacom-admin-open-auth-client.pages.listing.columns.provider")},{property:"userKeys.length",label:this.$t("heptacom-admin-open-auth-client.pages.listing.columns.users"),width:"100px"},{property:"createdAt",label:this.$t("heptacom-admin-open-auth-client.pages.listing.columns.createdAt"),width:"200px"}]}},created(){this.getList()},computed:{clientRepository(){return this.repositoryFactory.create("heptacom_admin_open_auth_client")},clientCriteria(){let e=new v,t=this.getMainListingParams();return e.addAssociation("userKeys"),e.setLimit(t.limit),e.setPage(t.page),e.addSorting(v.sort(t.sortBy||"name",t.sortDirection||"ASC")),t.term&&t.term.length&&e.addFilter(v.contains("name",t.term)),e}},methods:{getList(){return this.loadData()},loadData(){this.isLoading=!0,this.loadClients().then(()=>{this.isLoading=!1})},loadClients(){return this.items=null,this.clientRepository.search(this.clientCriteria,xt.api).then(e=>{this.items=e})},getLoginColor(e){return e.active?e.login?"#00cc00":"#cc0000":"#333333"},getConnectColor(e){return e.active?e.connect?"#00cc00":"#cc0000":"#333333"}}}});var he,me=o(()=>{he=`{% block heptacom_admin_open_auth_provider_cidaas_settings %}
...
{% endblock %}
`});var ue={};a(ue,{default:()=>Lt});var Lt,_e=o(()=>{me();Lt={template:he,props:{item:{required:!0}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var fe,ge=o(()=>{fe=`{% block heptacom_admin_open_auth_client_google_cloud_settings %}
...
{% endblock %}
`});var ve={};a(ve,{default:()=>Tt});var Tt,be=o(()=>{ge();Tt={template:fe,props:{item:{required:!0}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var Ae,we=o(()=>{Ae=`{% block heptacom_admin_open_auth_provider_jira_settings %}
...
{% endblock %}
`});var ye={};a(ye,{default:()=>Ht});var Ht,ke=o(()=>{we();Ht={template:Ae,props:{item:{required:!0}}}});var xe,Ce=o(()=>{xe=`{% block heptacom_admin_open_auth_provider_jumpcloud_settings %}
    ...
    {% endblock %}
{% endblock %}
`});var Oe={};a(Oe,{default:()=>Nt});var Nt,Se=o(()=>{Ce();Nt={template:xe,props:{item:{required:!0}},data(){return{availableProperties:["firstName","lastName","email","timezone","locale","roles"]}},watch:{item(e){e.config.attributeMapping||(e.config.attributeMapping={})}}}});var Pe,$e=o(()=>{Pe=`{% block heptacom_admin_open_auth_provider_keycloak_settings %}
...
{% endblock %}
`});var Fe={};a(Fe,{default:()=>zt});var zt,Re=o(()=>{$e();zt={template:Pe,props:{item:{required:!0}},data(){return{jsonPlaceholder:JSON.stringify({realm:"master","auth-server-url":"https://keycloak.my-company.com/auth/","ssl-required":"external",resource:"my-client",credentials:{secret:"abcdefghijgklmnopqrstuvwxyz"},"confidential-port":0},null,"	")}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var Ue,Me=o(()=>{Ue=`{% block heptacom_admin_open_auth_provider_microsoft_azure_oidc_settings %}
...
{% endblock %}
`});var qe={};a(qe,{default:()=>Kt});var Kt,Le=o(()=>{Me();Kt={template:Ue,props:{item:{required:!0}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var Te,De=o(()=>{Te=`{% block heptacom_admin_open_auth_provider_okta_settings %}
...
{% endblock %}
`});var Ie={};a(Ie,{default:()=>Jt});var Jt,He=o(()=>{De();Jt={template:Te,props:{item:{required:!0}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var Ne,Ee=o(()=>{Ne=`{% block heptacom_admin_open_auth_provider_onelogin_settings %}
...
{% endblock %}
`});var je={};a(je,{default:()=>Vt});var Vt,ze=o(()=>{Ee();Vt={template:Ne,props:{item:{required:!0}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var Ke,Be=o(()=>{Ke=`{% block sw_condition_value_content %}
...
{% endblock %}
`});var Xe=o(()=>{});var Je={};a(Je,{default:()=>Yt});var Zt,Qt,Yt,Ge=o(()=>{Be();Xe();({Component:Zt}=Shopware),{mapPropertyErrors:Qt}=Zt.getComponentHelper(),Yt={template:Ke,computed:{requestUrl:{get(){return this.ensureValueExist(),this.condition.value.requestUrl||null},set(e){this.ensureValueExist(),this.condition.value={...this.condition.value,requestUrl:e}}},jmesPathExpression:{get(){return this.ensureValueExist(),this.condition.value.jmesPathExpression||null},set(e){this.ensureValueExist(),this.condition.value={...this.condition.value,jmesPathExpression:e}}},...Qt("condition",["value.requestUrl","value.jmesPathExpression"]),currentError(){return this.conditionValueRequestUrlError||this.conditionValueJmesPathExpressionError}}}});var We,Ve=o(()=>{We=`{% block heptacom_admin_open_auth_provider_open_id_connect_settings %}
...
{% endblock %}
`});var Ze={};a(Ze,{default:()=>ti});var ti,Qe=o(()=>{Ve();ti={template:We,props:{item:{required:!0}},watch:{item(e){e.config.scopes||(e.config.scopes=[])}}}});var et,Ye=o(()=>{et=`{% block heptacom_admin_open_auth_provider_saml2_settings %}
    <div>
...
{% endblock %}
`;var{Component:ut}=Shopware;ut.override("sw-verify-user-modal",{template:q,inject:["systemConfigApiService"],data(){return{denyPasswordLogin:!1}},created(){this.systemConfigApiService.getValues("KskHeptacomAdminOpenAuth.config").then(e=>{this.denyPasswordLogin=e["KskHeptacomAdminOpenAuth.config.denyPasswordLogin"]})},methods:{onCloseConfirmPasswordModal(){return this.$refs.heptacomAdminOpenAuthUserConfirmLogin.abortAuthFlow(),this.$super("onCloseConfirmPasswordModal")},heptacomAdminOpenAuthUserConfirm(e){this.$emit("verified",e)}}});Shopware.Service("privileges").addPrivilegeMappingEntry({category:"permissions",parent:"settings",key:"heptacom_admin_open_auth_client",roles:{viewer:{privileges:["heptacom_admin_open_auth_client:read","heptacom_admin_open_auth_user_key:read","heptacom_admin_open_auth_client_rule:read","heptacom_admin_open_auth_client_rule_condition:read"],dependencies:[]},editor:{privileges:["heptacom_admin_open_auth_client:update","heptacom_admin_open_auth_client_rule:create","heptacom_admin_open_auth_client_rule:update","heptacom_admin_open_auth_client_rule:delete","heptacom_admin_open_auth_client_rule_condition:create","heptacom_admin_open_auth_client_rule_condition:update","heptacom_admin_open_auth_client_rule_condition:delete"],dependencies:["heptacom_admin_open_auth_client.viewer"]},creator:{privileges:["heptacom_admin_open_auth_client:create"],dependencies:["heptacom_admin_open_auth_client.editor"]},deleter:{privileges:["heptacom_admin_open_auth_client:delete"],dependencies:["heptacom_admin_open_auth_client.viewer"]}}});var{Component:p,Module:Pt}=Shopware;p.register("heptacom-admin-open-auth-client-rule-container",()=>Promise.resolve().then(()=>(I(),T)));p.register("heptacom-admin-open-auth-client-rule-item",()=>Promise.resolve().then(()=>(z(),j)));p.register("heptacom-admin-open-auth-client-create-page",()=>Promise.resolve().then(()=>(G(),J)));p.register("heptacom-admin-open-auth-client-edit-page",()=>Promise.resolve().then(()=>(te(),ee)));p.register("heptacom-admin-open-auth-client-listing-page",()=>Promise.resolve().then(()=>(re(),ae)));Pt.register("heptacom-admin-open-auth-client",{type:"plugin",name:"heptacom-admin-open-auth-client.module.name",title:"heptacom-admin-open-auth-client.module.title",description:"heptacom-admin-open-auth-client.module.description",color:"#FFC2A2",icon:"regular-sign-in",routes:{create:{component:"heptacom-admin-open-auth-client-create-page",path:"create",meta:{parentPath:"heptacom.admin.open.auth.client.settings",privilege:"heptacom_admin_open_auth_client.creator"}},edit:{component:"heptacom-admin-open-auth-client-edit-page",path:"edit/:id",meta:{parentPath:"heptacom.admin.open.auth.client.settings",privilege:"heptacom_admin_open_auth_client.editor"},props:{default(e){return{clientId:e.params.id}}}},settings:{component:"heptacom-admin-open-auth-client-listing-page",path:"settings",meta:{parentPath:"sw.settings.index",privilege:"heptacom_admin_open_auth_client.viewer"}}},settingsItem:[{to:"heptacom.admin.open.auth.client.settings",group:"system",icon:"regular-sign-in",privilege:"heptacom_admin_open_auth_client.viewer"}]});var se=e=>(e.addCondition("heptacomAdminOpenAuthMicrosoftAzureOidcGroups",{component:"sw-condition-generic",label:"heptacomAdminOpenAuthClient.providerFields.microsoft_azure_oidc.condition.rule.groupIds",scopes:["microsoft_azure_oidc"],group:"user"}),e);var le=e=>(e.addCondition("heptacomAdminOpenAuthAuthenticatedRequest",{component:"heptacom-admin-open-auth-condition-authenticated-request",label:"heptacomAdminOpenAuthClient.providerFields.open_id_connect.condition.rule.authenticatedRequest",scopes:["open_id_connect","cidaas","google_cloud","keycloak","microsoft_azure_oidc","okta","onelogin"],group:"user"}),e);var de=e=>(e.addCondition("heptacomAdminOpenAuthSaml2Role",{component:"sw-condition-generic",label:"heptacomAdminOpenAuthClient.providerFields.saml2.condition.rule.roles",scopes:["saml2"],group:"user"}),e);var ce=`{% block heptacom_admin_open_auth_client_edit_page_content_base_settings_inner_store_user_token %}
...
`;var{Component:Rt}=Shopware;Rt.override("heptacom-admin-open-auth-client-edit-page",{template:ce,data(){return{availableProperties:["firstName","lastName","email","timezone","locale"]}},watch:{item(e){e&&e.provider==="jumpcloud"&&(e.config.attributeMapping||(e.config.attributeMapping={}))}}});var pe=`{% block heptacom_admin_open_auth_client_edit_page_content_base_settings_inner_store_user_token %}
